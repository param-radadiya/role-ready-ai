...
import { LayoutDashboard, AlertCircle, RefreshCw, Linkedin } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20 font-sans selection:bg-violet-100 selection:text-violet-900">
      {/* Header */}
      <header className="bg-white/80 backdrop-blur-md border-b border-slate-200 sticky top-0 z-20">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="bg-gradient-to-br from-violet-600 to-indigo-600 p-2 rounded-xl shadow-lg shadow-violet-200">
              <LayoutDashboard className="w-6 h-6 text-white" />
            </div>
            <div>
              <h1 className="text-xl font-bold text-slate-900 tracking-tight">RoleReadyAI</h1>
              <p className="text-xs text-slate-500 font-medium">AI-Powered Application Accelerator</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <a 
              ...
              className="group flex items-center gap-2 px-4 py-2 rounded-full bg-slate-100 hover:bg-violet-50 text-slate-700 hover:text-violet-700 text-sm font-semibold transition-all"
            >
              <span>Contact</span>
              <Linkedin className="w-4 h-4 text-slate-400 group-hover:text-violet-600 transition-colors" />
            </a>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-10">
        ...
        <div className="mb-10 text-center max-w-2xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <p className="text-slate-600 text-lg leading-relaxed">
            Select your goal below to tailor your resume or prepare for the interview using <span className="text-violet-600 font-semibold">Gemini AI</span>.
          </p>
        </div>

        {/* Inputs */}
        <div className="relative">
          <div className="absolute inset-0 bg-gradient-to-r from-violet-100/50 to-indigo-100/50 blur-3xl -z-10 transform -skew-y-3"></div>
          <InputSection 
            jobDesc={jobDesc}
        ...
        {result && (
          <div ref={resultsRef} className="space-y-10 mt-16 pt-10 border-t border-slate-200 animate-in fade-in slide-in-from-bottom-8">
            <div className="text-center mb-8">
              <span className="bg-violet-100 text-violet-800 text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider">Result</span>
      ...
      <footer className="text-center py-12 text-slate-500 text-sm border-t border-slate-200 mt-20 bg-white">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <p className="flex items-center justify-center gap-1.5">
            Built by <a href="https://www.linkedin.com/in/param-radadiya-77a4b51a6/" target="_blank" rel="noopener noreferrer" className="text-violet-600 hover:text-violet-800 font-bold hover:underline transition-all">Param Radadiya</a>
          </p>
        </div>
      </footer>
    </div>
